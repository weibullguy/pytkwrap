"""Test module for the GTK3TreeStore class.
...
from pytkwrap.gtk3.treeview import GTK3TreeStore
...
from tests.gtk3.treeview.constants import (
    EXPECTED_TREESTORE_ATTRIBUTES,
    EXPECTED_TREESTORE_METHODS,
)


@pytest.mark.usefixtures("suppress_stderr")
class TestGTK3TreeStore(BaseGTK3GObjectTests):
    """Test class for the GTK3TreeStore class."""

    widget_class = GTK3TreeStore
    expected_attributes = EXPECTED_GOBJECT_ATTRIBUTES | EXPECTED_TREESTORE_ATTRIBUTES
    expected_handler_id = EXPECTED_GOBJECT_HANDLER_IDS
    expected_methods = EXPECTED_GOBJECT_METHODS + EXPECTED_TREESTORE_METHODS

    def make_dut(self, column_types=None, n_columns=0, n_rows=0):
        """Create a device under test for the GTK3TreeStore."""
        return self.widget_class(column_types, n_columns, n_rows)

    @pytest.mark.unit
    def test_init_with_column_types(self):
        """Should create a GTK3TreeStore with column types."""
    ...
    def test_init_with_n_columns(self):
        """Should create a GTK3TreeStore with n_columns."""
    ...
    def test_init_with_n_rows(self):
        """Should create a GTK3TreeStore with n_rows."""
    ...
    def test_init_with_column_types_and_zero_n_columns(self):
        """Should create a GTK3TreeStore with column types and n_columns."""
    ...
    def test_do_set_attributes(self):
        ...
        assert dut.get_column_type(0) == GObject.TYPE_STRING
        assert dut.get_column_type(1) == GObject.TYPE_INT
        assert dut.get_column_type(2) == GObject.TYPE_DOUBLE
        assert dut.do_get_attribute("n_columns") == 3
        assert dut.get_n_columns() == 3
        assert dut.do_get_attribute("n_rows") == 10
